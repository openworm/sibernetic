...
def parse_position_buffer(path: Path):
    ...
    if sidecar.exists():
        ...
        if len(frame_times) != len(frames):
            print(f'WARN: sidecar has {len(frame_times)} times but {len(frames)} '
...
def check_parity(ref_path: Path, cand_path: Path, *, n_samples: int = 30,
...
def main():
    ...
    ap.add_argument('reference', type=Path,
                    help='OpenCL gold-standard position_buffer.txt')
    ap.add_argument('candidate', type=Path,
                    help='Metal substrate position_buffer.txt')
    ...
    print(f"Backend parity check on demo1")
    print(f"  reference: {r['ref_path']}")
    print(f"  candidate: {r['cand_path']}")
    print(f"  particles: {r['n_active_particles']}  sim_t: 0 → {r['sim_time_s']:.2f}s  samples: {r['n_samples']}")
    print()
    print(f"  {chk(r['init_ok'])} initial per-particle L2 = {r['init_l2']:.3f} "
          f"(threshold < {THRESHOLDS['init_l2_max']})")
    print(f"  {chk(r['mean_ok'])} mean per-particle L2 over time = {r['mean_per_particle_l2']:.3f} "
          f"(threshold < {THRESHOLDS['mean_per_particle_l2_max']})")
    print(f"  {chk(r['center_ok'])} max cube-center diff = {r['max_cube_center_diff']:.3f} "
          f"(threshold < {THRESHOLDS['cube_center_diff_max']})")
    print(f"  {chk(r['extent_ok'])} max cube-extent diff = {r['max_cube_extent_diff']:.3f} "
          f"(threshold < {THRESHOLDS['cube_extent_diff_max']})")
    print(f"  {chk(r['final_ok'])} final per-particle L2 = {r['final_l2']:.3f} "
          f"(threshold < {THRESHOLDS['final_l2_max']})")
    print()
    if r['all_ok']:
        print(f"  [OVERALL PASS] backends agree on demo1 within tolerance")
    else:
        print(f"  [OVERALL FAIL] substrates have diverged on demo1")
    return 0 if r['all_ok'] else 1
